...
public class SitAndReachActivity extends NFCActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (items == null) {
			...
			max = Integer.parseInt(items.getMaxValue().toString()) / 10 + "";
			min = Integer.parseInt(items.getMinValue().toString()) / 10 + "";
		}
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(etChengji.getText().toString()) && checkedBtn.equals("正常")) {
					...
					// 查询数据库中保存的该学生项目成绩的轮次
					// String itemCode =
					// DbService.getInstance(context).queryItemByMachineCode(Constant.SIT_AND_REACH
					// + "")
					// .getItemCode();
					// studentItems =
