...
public class StudentItem {
    ...
    private List<TotalRoundResult> totalRoundResults;
    ...
    /** To-many relationship, resolved on first access (and after reset). Changes to to-many relations are not persisted, make changes to the target entity. */
    public List<TotalRoundResult> getTotalRoundResults() {
        if (totalRoundResults == null) {
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            TotalRoundResultDao targetDao = daoSession.getTotalRoundResultDao();
            List<TotalRoundResult> totalRoundResultsNew = targetDao._queryStudentItem_TotalRoundResults(StudentItemID);
            synchronized (this) {
                if(totalRoundResults == null) {
                    totalRoundResults = totalRoundResultsNew;
                }
            }
        }
        return totalRoundResults;
    }

    /** Resets a to-many relationship, making the next get call to query for a fresh result. */
    public synchronized void resetTotalRoundResults() {
        totalRoundResults = null;
    }
